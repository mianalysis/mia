    ...
    */
    ...
    public static void calculateTemporalMeasurements(ObjI trackObject, Track track, boolean averageSubtracted) {
        if (track.size() == 0) {
            ...
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, Double.NaN));
            ...
            String name = getFullName(Measurements.DURATION, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, duration));
            name = getFullName(Measurements.FIRST_FRAME, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, firstPoint.getF()));
            name = getFullName(Measurements.LAST_FRAME, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, lastPoint.getF()));

            int nSpots = track.values().size();
            double detectionFraction = (double) nSpots / ((double) duration + 1);
            name = getFullName(Measurements.DETECTION_FRACTION, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, detectionFraction));
        }
    }
    ...
    public static void calculateVelocity(ObjI trackObject, Track track, boolean averageSubtracted) {
        if (track.size() <= 1) {
            ...
            String name = getFullName(Measurements.MEAN_X_VELOCITY_PX, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, cumStatX.getMean()));
            name = getFullName(Measurements.MEAN_X_VELOCITY_CAL, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, cumStatX.getMean() * distPerPxXY));
            name = getFullName(Measurements.MEAN_Y_VELOCITY_PX, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, cumStatY.getMean()));
            name = getFullName(Measurements.MEAN_Y_VELOCITY_CAL, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, cumStatY.getMean() * distPerPxXY));
            name = getFullName(Measurements.MEAN_Z_VELOCITY_SLICES, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, cumStatZ.getMean() * distPerPxXY / distPerPxZ));
            name = getFullName(Measurements.MEAN_Z_VELOCITY_CAL, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, cumStatZ.getMean() * distPerPxXY));
            name = getFullName(Measurements.MEAN_Z_VELOCITY_SLICES, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, cumStatZ.getMean() * distPerPxXY / distPerPxZ));
            name = getFullName(Measurements.MEAN_Z_VELOCITY_CAL, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, cumStatZ.getMean() * distPerPxXY));
            name = getFullName(Measurements.MEAN_INSTANTANEOUS_SPEED_PX, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, cumStatSpeed.getMean()));
            name = getFullName(Measurements.MEAN_INSTANTANEOUS_SPEED_CAL, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, cumStatSpeed.getMean() * distPerPxXY));
    ...
    public static void calculateRelativeTimepoint(ObjI trackObject, Track track, String inputSpotObjectsName,
        ...
        for (ObjI spotObject : trackObject.getChildren(inputSpotObjectsName).values()) {
            ...
            spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, currentTimepoint - firstTimepoint));
    ...
    public static void calculateSpatialMeasurements(ObjI trackObject, Track track, boolean averageSubtracted) {
        if (track.size() == 0) {
            ...
            String name = getFullName(Measurements.EUCLIDEAN_DISTANCE_PX, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, euclideanDistance));
            name = getFullName(Measurements.EUCLIDEAN_DISTANCE_CAL, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, euclideanDistance * distPerPxXY));
            name = getFullName(Measurements.TOTAL_PATH_LENGTH_PX, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, totalPathLength));
            name = getFullName(Measurements.TOTAL_PATH_LENGTH_CAL, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, totalPathLength * distPerPxXY));
            name = getFullName(Measurements.DIRECTIONALITY_RATIO, averageSubtracted);
            trackObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, euclideanDistance / totalPathLength));
    ...
    public static void calculateInstantaneousVelocity(ObjI trackObject, Track track, String inputSpotObjectsName,
        ...
        for (ObjI spotObject : trackObject.getChildren(inputSpotObjectsName).values()) {
            ...
            if (t == minT) {
                ...
                spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, Double.NaN));
                ...
                String name = getFullName(Measurements.X_VELOCITY_PX, averageSubtracted);
                spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, xVelocity.get(t)));
                name = getFullName(Measurements.X_VELOCITY_CAL, averageSubtracted);
                spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, xVelocity.get(t) * distPerPxXY));
                name = getFullName(Measurements.Y_VELOCITY_PX, averageSubtracted);
                spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, yVelocity.get(t)));
                name = getFullName(Measurements.Y_VELOCITY_CAL, averageSubtracted);
                spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, yVelocity.get(t) * distPerPxXY));
                name = getFullName(Measurements.Z_VELOCITY_SLICES, averageSubtracted);
                spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, zVelocity.get(t) * distPerPxXY / distPerPxZ));
                name = getFullName(Measurements.Z_VELOCITY_CAL, averageSubtracted);
                spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, zVelocity.get(t) * distPerPxXY));
                name = getFullName(Measurements.INSTANTANEOUS_SPEED_PX, averageSubtracted);
                spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, speed.get(t)));
                name = getFullName(Measurements.INSTANTANEOUS_SPEED_CAL, averageSubtracted);
                spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, speed.get(t) * distPerPxXY));
    ...
    public static void calculateInstantaneousSpatialMeasurements(ObjI trackObject, Track track,
        ...
        for (ObjI spotObject : trackObject.getChildren(inputSpotObjectsName).values()) {
            ...
            spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, pathLength.get(t)));
            name = getFullName(Measurements.CUMULATIVE_PATH_LENGTH_CAL, averageSubtracted);
            spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, pathLength.get(t) * distPerPxXY));
            name = getFullName(Measurements.ROLLING_EUCLIDEAN_DISTANCE_PX, averageSubtracted);
            spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, euclidean.get(t)));
            name = getFullName(Measurements.ROLLING_EUCLIDEAN_DISTANCE_CAL, averageSubtracted);
            spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, euclidean.get(t) * distPerPxXY));
            name = getFullName(Measurements.ROLLING_DIRECTIONALITY_RATIO, averageSubtracted);
            spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, dirRatio.get(t)));
            name = getFullName(Measurements.ANGULAR_PERSISTENCE, averageSubtracted);
            spotObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, angularPersistence.get(t)));
    ...
    public static void calculateOrientation(ObjsI objects, String orientationMode, boolean averageSubtracted) {
        ...
        for (ObjI obj : objects.values()) {
            ...
            if (Double.isNaN(angle))
                obj.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, Double.NaN));
            else
                obj.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, Math.toDegrees(angle)));
    ...
    public static void identifyLeading(ObjsI objects, String orientationMode, boolean averageSubtracted) {
        for (ObjI obj : objects.values()) {
            ...
            if (Double.isNaN(angle)) {
                ...
                // Adding furthest point coordinates to measurements
                name = getFullName(Measurements.LEADING_X_PX, averageSubtracted);
                obj.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, furthestPoint.getX()));

                name = getFullName(Measurements.LEADING_Y_PX, averageSubtracted);
                obj.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, furthestPoint.getY()));

                name = getFullName(Measurements.LEADING_Z_PX, averageSubtracted);
                obj.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(name, 0));
