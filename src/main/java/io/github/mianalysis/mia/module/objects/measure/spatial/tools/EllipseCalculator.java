...
public class EllipseCalculator {
    ...
    public EllipseCalculator(Volume volume, double maxAxisLength) throws RuntimeException {
        MIA.log.writeDebug(volume.is2D()+"_"+volume.getZMean(true, false));
        if (!volume.is2D())
            this.volume = volume.getProjected();
        else
            this.volume = volume;
    ...
    // /**
    //  * Returns the semi-major and semi-minor axes. Note: There's no guarantee which
    ...
    //     // If the semiaxes come off in reverse order (i.e. [0] semi-minor, [1]
    //     // semi-major) reverse the sign
    //     double[] semiAxes = getSemiAxes();
