...
public class ExtractObjectEdges extends Module {
	...
	*/
    ...
    public interface EdgeModes {
        ...
        String[] ALL = new String[]{DISTANCE_FROM_EDGE, OBJECT_MEASUREMENT, PARENT_OBJECT_MEASUREMENT, PERCENTAGE_FROM_EDGE};
    ...
    public Parameters updateAndGetParameters() {
        ...
        switch ((String) parameters.getValue(EDGE_MODE, workspace)) {
            ...
            case EdgeModes.OBJECT_MEASUREMENT:
                ...
                ((ObjectMeasurementP) parameters.getParameter(MEASUREMENT_NAME)).setObjectName(parameters.getValue(INPUT_OBJECTS, workspace));
                returnedParameters.add(parameters.getParameter(CALIBRATED_DISTANCES));
                break;
            case EdgeModes.PARENT_OBJECT_MEASUREMENT:
                returnedParameters.add(parameters.getParameter(PARENT_OBJECTS));
                ((ParentObjectsP) parameters.getParameter(PARENT_OBJECTS)).setChildObjectsName(parameters.getValue(INPUT_OBJECTS, workspace));
                returnedParameters.add(parameters.getParameter(PARENT_MEASUREMENT_NAME));
                ((ObjectMeasurementP) parameters.getParameter(PARENT_MEASUREMENT_NAME)).setObjectName(parameters.getValue(PARENT_OBJECTS, workspace));
                returnedParameters.add(parameters.getParameter(CALIBRATED_DISTANCES));
                break;
            case EdgeModes.PERCENTAGE_FROM_EDGE:
    ...
    void addParameterDescriptions() {
        ...
        + "<li>\"" + EdgeModes.OBJECT_MEASUREMENT + "\" The boundary is defined by a measurement value associated with each object.  The measurement is specified by \""+MEASUREMENT_NAME+"\".  Any input object coordinates within (less than or equal to) this distance of the object edge can be output as \"edge\" coordinates, otherwise they can be output as \"interior\" coordinates.</li>"

        + "<li>\"" + EdgeModes.PARENT_OBJECT_MEASUREMENT + "\" The boundary is defined by a measurement value associated with a parent of each object.  The measurement is specified by \""+PARENT_MEASUREMENT_NAME+"\".  Any input object coordinates within (less than or equal to) this distance of the object edge can be output as \"edge\" coordinates, otherwise they can be output as \"interior\" coordinates.</li>"
        ...
        + "\", this is the object measurement (associated with the object being processed) that defines the boundary between edge and interior objects.  It is assumed to be specified in pixel units unless \""+CALIBRATED_DISTANCES+"\" is selected, in which case they are assumed in calibrated units.");
        
        parameters.get(PARENT_OBJECTS).setDescription("If \"" + EDGE_MODE + "\" is set to \""+ EdgeModes.PARENT_OBJECT_MEASUREMENT+ "\", this is the parent of the input object that will be used as a source for the edge width measurement specified by \""+ EdgeModes.PARENT_OBJECT_MEASUREMENT+ "\".");

        parameters.get(PARENT_MEASUREMENT_NAME).setDescription("If \"" + EDGE_MODE + "\" is set to \""
        + EdgeModes.PARENT_OBJECT_MEASUREMENT
        + "\", this is the object measurement (associated with a parent of the object being processed) that defines the boundary between edge and interior objects.  It is assumed to be specified in pixel units unless \""+CALIBRATED_DISTANCES+"\" is selected, in which case they are assumed in calibrated units.");
