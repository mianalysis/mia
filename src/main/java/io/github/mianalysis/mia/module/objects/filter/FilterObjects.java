...
public class FilterObjects extends Module implements ActionListener {
	...
	*/
	...
	/**
	* Controls the type of filter being used:<br><ul><li>"Exclude objects on image edge (2D)" Remove any objects based on contact with the image edge.  Contact is considered as a case where an object pixel is in the outer-most row, column or slice  of an image (e.g. x = 0 or y = max_value).  Can optionally also consider top and bottom of 3D stack (Z-position).</li><li>"Remove objects with missing measurements" Remove any objects without a specific measurement value (i.e. values with NaN values).</li><li>"Remove objects without parent" Remove any objects without an assigned parent from a specific object collection.</li><li>"Remove objects with a parent" Remove any objects with an assigned parent from a specific object collection.</li><li>"Remove objects with fewer children than:" Remove any objects with fewer children from a specific object collection than a threshold value.</li><li>"Remove objects with more children than:" Remove any objects with more children from a specific object collection than a threshold value.</li><li>"Remove objects with measurements less than:" Remove any objects with a smaller measurement value a specific threshold.</li><li>"Remove objects with measurements greater than:" Remove any objects with a larger measurement value a specific threshold.</li><li>"Specific object IDs (runtime)" Remove objects by their specific ID numbers.  This is a manual option, whereby an image is displayed to the user (pre-prepared to show object ID numbers) along with a dialog box allowing the user to write a comma-separated list of objects to remove.</li></ul>
	*/
	...
	*/
	...
	/**
	* Type of reference value used to compare objects to:<br><ul><li>"Fixed value" Objects will be compared to a single, fixed value specified using the "Reference value" parameter.</li><li>"Image measurement" Objects will be compared to a measurement associated with an image in the workspace.  The image and associated measurement are specified by the "Reference value image" and "Reference image measurement" parameters.  In this mode, all objects will still be compared to the same value; however, this value can vary between input files.</li><li>"Parent object measurement" Objects will be compared to a measurement associated with a parent object.  The parent object and measurement are specified by the "Reference value parent object" and "Reference object measurement" parameters.  In this mode all objects can (but won't necessarily) be compared to different values.</li></ul>
	*/
    public static final String REFERENCE_MODE = "Reference mode";

	/**
	* When "Reference mode" is set to "Fixed value", all objects will be compared to this fixed value.
	*/
    public static final String REFERENCE_VALUE = "Reference value";

	/**
	* When "Reference mode" is set to "Image measurement", all objects will be compared to a measurement associated with this image.
	*/
    public static final String REFERENCE_VAL_IMAGE = "Reference value image";

	/**
	* When "Reference mode" is set to "Image measurement", all objects will be compared to this measurement, which itself is associated with the image specified by "Reference value image".
	*/
    public static final String REFERENCE_IMAGE_MEASUREMENT = "Reference image measurement";

	/**
	* When "Reference mode" is set to "Parent object measurement", all objects will be compared to a measurement associated with their parent object from this collection.
	*/
    public static final String REFERENCE_VAL_PARENT_OBJECT = "Reference value parent object";

	/**
	* When "Reference mode" is set to "Parent object measurement", all objects will be compared to this measurement, which itself is associated with the parent object specified by "Reference value parent object".
	*/
    public static final String REFERENCE_OBJECT_MEASUREMENT = "Reference object measurement";

	/**
	* Irrespective of how the reference value was determined, it can be systematically adjusted prior to use for comparison using this multiplier.  For example, an image measurement reference value of 32 with a "Reference value multiplier" of "0.5" will see all objects compared against a value of 16.  This is useful when comparing to dynamic values coming from image and parent object measurements.
	*/
