...
public class ProjectImage<T extends RealType<T> & NativeType<T>> extends Module {
    ...
    // public Image project(Image inputImage, String projectionDimension, String
    // projectionMode) {
    ...
    // inputImg);
    //
    // Img<T> projection=(Img<T>)ij.op().transform().project(proj, inputImg,
    // mean_op, dim);
    //
    ...
    protected void initialiseParameters() {
        ...
        parameters.add(new OutputImageP(OUTPUT_IMAGE, this, "",
                "Name for the projected image to be stored in the workspace with."));
        ...
        parameters.add(new ChoiceP(PROJECTION_MODE, this, ProjectionModes.MAX, ProjectionModes.ALL,
                "Statistic for calculation of the intensity projection."));
...
///// THE FOLLOWING IS POTENTIALLY READY TO USE; IT JUST NEEDS LOADS OF TESTS
///// WRITING FIRST /////
...
// * Created by sc13967 on 04/05/2017.
// */
// public class ProjectImage <T extends RealType<T> & NativeType<T>> extends
// Module {
// public static final String INPUT_SEPARATOR = "Image input/output";
// public static final String INPUT_IMAGE = "Input image";
// public static final String OUTPUT_IMAGE = "Output image";
...
// }
//
// public static <T extends RealType<T> & NativeType<T>> Image
// projectImageInZ(Image inputImage, String outputImageName, String
// projectionMode) {
// return
// project(inputImage,outputImageName,AxisModes.X,AxisModes.Y,AxisModes.Z,projectionMode);
// }
//
// public static <T extends RealType<T> & NativeType<T>> Image project(Image
// inputImage, String outputImageName, String outputXAxis, String outputYAxis,
...
// if (img.dimensionIndex(projType) == -1) return
// getNonProjectedImage(img,outputImageName,xType,yType);
//
...
// ImgPlus<T> proj = ImgPlus.wrap((Img<T>) ij.op().create().img(new
// FinalDimensions(projected_dimensions), perm.randomAccess().get()));
//
// // Apply transformation
// UnaryComputerOp mean_op = (UnaryComputerOp)
// ij.op().op(getProjection(projectionMode),img);
...
// }
//
// static <T extends RealType<T> & NativeType<T>> Image
// getNonProjectedImage(ImgPlus<T> img, String outputImageName, AxisType xType,
// AxisType yType) {
// ImageJ ij = new ImageJ();
//
// HashMap<Integer,AxisType> axisAssignments = getAxisAssignments(img);
...
// enforceCZT(RandomAccessibleInterval<T> imgPlus, HashMap<Integer,AxisType>
// axisAssignments) {
// // This problem should only arise with 5 dimensions, as the first 3 are fixed
// by the projection configuration
// if (imgPlus.numDimensions() == 5) {
// if (axisAssignments.get(3) == Axes.Z && axisAssignments.get(4) ==
// Axes.CHANNEL) {
// return permute(imgPlus,axisAssignments,Axes.Z,4);
// } else if (axisAssignments.get(3) == Axes.TIME && axisAssignments.get(4) ==
// Axes.CHANNEL) {
...
// }
//
// static <T extends RealType<T> & NativeType<T>> HashMap<Integer,AxisType>
// getAxisAssignments(ImgPlus<T> img) {
...
// MIA.log.writeWarning("Specified axis ("+type.getLabel()+") not present in
// image");
// return false;
// }
//
...
//
// static <T extends RealType<T> & NativeType<T>> RandomAccessibleInterval<T>
// permute(RandomAccessibleInterval<T> interval, HashMap<Integer,AxisType>
// axisAssignments, AxisType fromType, int toIdx) {
...
// }
//
// static int getPosition(HashMap<Integer,AxisType> axisAssignments, AxisType
// type) {
...
// // Create max projection image
// Image outputImage =
// project(inputImage,outputImageName,xAxis,yAxis,projectionAxis,projectionMode);
//
// // Adding projected image to workspace
// workspace.addImage(outputImage);
//
...
// parameters.add(new
// ChoiceP(PROJECTION_MODE,this,ProjectionModes.AVERAGE,ProjectionModes.ALL));
//
// }
//
// @Override
