// TODO: Composite images (i.e. ensure output is composite, or add option)
// TODO: Overlay flattening and RGB conversion
...
public class SaveImage extends AbstractImageSaver {
    ...
    public SaveImage(Modules modules) {
        super("Save image 2", modules);
    ...
    public Status process(Workspace workspace) {
        ...
        ImagePlus ipl = ImageJFunctions.wrap(inputImage.getImgPlus(), inputImage.getName());

        switch (fileFormat) {
            case FileFormats.AVI:
                path = path + suffix + ".avi";
                saveVideo(ipl, compressionMode, frameRate, quality, path);
                break;
            case FileFormats.TIF:
            case FileFormats.ZIP:
                path = path + suffix + ".tif";
                saveImage(ipl, fileFormat, path);
                break;
        }
