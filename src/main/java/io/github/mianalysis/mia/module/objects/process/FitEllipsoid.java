...
// import
// io.github.mianalysis.mia.module.objects.measure.spatial.tools.EllipsoidCalculator;
...
// public void processObject(Obj inputObject, Objs outputObjects, String
// objectOutputMode, String fittingMode,
...
// Obj edgeObject = GetObjectSurface.getSurface(inputObject, tempObjects,
// false);
...
// public Obj createNewObject(Obj inputObject, Volume ellipsoid, Objs
// outputObjects) {
// if (ellipsoid == null)
// return null;

// Obj ellipsoidObject =
// outputObjects.createAndAddNewObject(inputObject.getVolumeType());
...
// {
// if (calculator == null) {
// inputObject.addMeasurement(new Measurement(Measurements.X_CENT_PX,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.X_CENT_CAL,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.Y_CENT_PX,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.Y_CENT_CAL,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.Z_CENT_SLICE,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.Z_CENT_CAL,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.RADIUS_1_PX,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.RADIUS_1_CAL,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.RADIUS_2_PX,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.RADIUS_2_CAL,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.RADIUS_3_PX,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.RADIUS_3_CAL,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.SURFACE_AREA_PX,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.SURFACE_AREA_CAL,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.VOLUME_PX,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.VOLUME_CAL,
// Double.NaN));
// inputObject.addMeasurement(new Measurement(Measurements.ORIENTATION_1,
// Math.toDegrees(Double.NaN)));
// inputObject.addMeasurement(new Measurement(Measurements.ORIENTATION_2,
// Math.toDegrees(Double.NaN)));
...
// return "Fit ellipsoids to all objects in a collection using \"<a
// href=\"https://imagej.net/BoneJ\">BoneJ</a>\". FFit ellipsoids can be stored
// either as new objects, or replacing the input object
// coordinates.<br><br>Note: If updating input objects with ellipsoid
// coordinates, measurements associated with the input object (e.g. spatial
// measurements) will still be available, but may no longer be valid.";
// }
...
// double maxAxisLength = limitAxisLength ?
// parameters.getValue(MAXIMUM_AXIS_LENGTH) : Double.MAX_VALUE;
...
// // Running through each object, taking measurements and adding new object to
// the
...
// Runnable task = () -> {
// try {
// processObject(inputObject, finalOutputObjects, objectOutputMode, fittingMode,
// maxAxisLength);
// } catch (IntegerOverflowException e) {
// MIA.log.writeWarning("Integer overflow exception for object " +
// inputObject.getID()
...
// parameters.add(new ChoiceP(FITTING_MODE, this, FittingModes.FIT_TO_SURFACE,
// FittingModes.ALL));
...
// parameters.add(new ChoiceP(OBJECT_OUTPUT_MODE, this,
// OutputModes.DO_NOT_STORE, OutputModes.ALL));
...
// ObjMeasurementRef reference =
// objectMeasurementRefs.getOrPut(Measurements.X_CENT_PX);
...
// String inputObjectsName = parameters.getValue(INPUT_OBJECTS);
// String outputObjectsName = parameters.getValue(OUTPUT_OBJECTS);
// returnedRelationships.add(parentChildRefs.getOrPut(inputObjectsName,
// outputObjectsName));
...
// parameters.get(INPUT_OBJECTS).setDescription("Objects from workspace to which
// ellipsoids will be fit. Measurements made by this module are associated with
// these input objects, irrespective of whether the fit ellipsoids are also
// stored as objects.");

// parameters.get(FITTING_MODE).setDescription("Controls which object
// coordinates are used for ellipsoid fitting:<br><ul>"

// +"<li>\""+FittingModes.FIT_TO_WHOLE+"\" All coordinates for the input object
// are passed to the ellipsoid fitter.</li>"

// +"<li>\""+FittingModes.FIT_TO_SURFACE+"\" (default) Only surface coordinates
// of the input object are passed to the ellipsoid fitter. Surface coordinates
// are calculated using 6-way connectivity.</li></ul>");

// parameters.get(LIMIT_AXIS_LENGTH).setDescription("When selected, all axes of
// the the fit ellipsoids must be shorter than the length specified by
// \""+MAXIMUM_AXIS_LENGTH+"\". This helps filter out mis-fit ellipsoids and
// prevents unnecessary, massive memory use when storing ellipsoids.");

// parameters.get(MAXIMUM_AXIS_LENGTH).setDescription("Maximum length of any fit
// ellipsoid axis as measured in pixel units. This is onyl used if
// \""+LIMIT_AXIS_LENGTH+"\" is selected.");

// parameters.get(OBJECT_OUTPUT_MODE).setDescription("Controls whether the fit
// ellipsoid is stored as an object in the workspace:<br><ul>"

// +"<li>\""+OutputModes.CREATE_NEW_OBJECT+"\" Fit ellipsoids are stored as new
// objects in the workspace (name specified by \""+OUTPUT_OBJECTS+"\").
// Ellipsoids are \"solid\" objects, irrespective of whether they were only fit
// to input object surface coordinates. Ellipsoid objects are children of the
// input objects to which they were fit. If outputting ellipsoid objects, any
// measurements are still only applied to the corresponding input objects.</li>"

// +"<li>\""+OutputModes.DO_NOT_STORE+"\" (default) The ellipsoid coordinates
// are not stored.</li>"

// +"<li>\""+OutputModes.UPDATE_INPUT+"\" The coordinates of the input object
// are removed and replaced with the fit ellipsoid coordinates. Note:
// Measurements associated with the input object (e.g. spatial measurements)
// will still be available, but may no longer be valid.</li></ul>");

// parameters.get(OUTPUT_OBJECTS).setDescription("Name assigned to output
// ellipsoid objects if \""+OBJECT_OUTPUT_MODE+"\" is in
// \""+OutputModes.CREATE_NEW_OBJECT+"\" mode.");

// parameters.get(ENABLE_MULTITHREADING).setDescription("Process multiple input
// objects simultaneously. This can provide a speed improvement when working on
