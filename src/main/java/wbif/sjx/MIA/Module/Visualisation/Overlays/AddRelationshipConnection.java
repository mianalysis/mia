...
public class AddRelationshipConnection extends Overlay {
    ...
    double lineWidth, HashMap<Integer, Float> hues, double opacity, boolean renderInAllFrames,
    boolean multithread) {
        // Adding the overlay element
        try {
            // If necessary, turning the image into a HyperStack (if 2 dimensions=1 it will be a standard ImagePlus)
            ...
            for (Obj partnerObject1 : partnerObjects1.values()) {
                ...
                Runnable task = () -> {
                    ...
                    addPartnerOverlay(partnerObject1, partnerObjects2Name, finalIpl, colour, lineWidth,
                    renderInAllFrames);
    ...
    public static void addPartnerOverlay(Obj partnerObject1, String partnerObjects2Name, ImagePlus ipl, Color colour, double lineWidth, boolean renderInAllFrames) {
        if (ipl.getOverlay() == null) ipl.setOverlay(new ij.gui.Overlay());
        ...
        for (Obj partnerObject2:partnerObject1.getPartners(partnerObjects2Name).values()) {
            drawLine(ipl, partnerObject1, partnerObject2, t, colour, lineWidth);
        }
    }
    ...
    protected boolean process(Workspace workspace) {
        ...
        switch (lineMode) {
            ...
            addPartnerOverlay(ipl, partnerObjects1, partnerObjects2Name, lineWidth, hues, opacity, renderInAllFrames, multithread);
            break;
