...
public abstract class Overlay extends Module {
    ...
    void addAbstractParameterDescriptions() {
        parameters.get(COLOUR_MODE).setDescription("Method for determining colour of each object's corresponding overlay:<br>"
        ...
        parameters.get(SINGLE_COLOUR).setDescription("Colour for all object overlays to be rendered using.  This parameter is used if \"" + COLOUR_MODE
        + "\" is set to \""+ColourModes.SINGLE_COLOUR+"\".  Options are: "+String.join(", ", SingleColours.ALL)+".");

        parameters.get(CHILD_OBJECTS_FOR_COLOUR).setDescription("Object collection used to determine the colour of the overlay based on number of children per object when \"" + COLOUR_MODE
        + "\" is set to \"" + ColourModes.CHILD_COUNT
        + "\".  These objects will be children of the input objects.");

        parameters.get(MEASUREMENT_FOR_COLOUR).setDescription("Measurement used to determine the colour of the overlay when \"" + COLOUR_MODE
        + "\" is set to either \"" + ColourModes.MEASUREMENT_VALUE+ "\" or \""+ColourModes.PARENT_MEASUREMENT_VALUE+"\".");

        parameters.get(PARENT_OBJECT_FOR_COLOUR).setDescription("Object collection used to determine the colour of the overlay based on either the ID or measurement value "
        ...
        parameters.get(PARTNER_OBJECTS_FOR_COLOUR).setDescription("Object collection used to determine the colour of the overlay based on number of partners per object when \"" + COLOUR_MODE
        + "\" is set to \"" + ColourModes.PARTNER_COUNT
        + "\".  These objects will be partners of the input objects.");

        parameters.get(OPACITY).setDescription("Opacity of the overlay to be rendered.  This is a value between 0 (totally transparent) and 100 (totally opaque).");
