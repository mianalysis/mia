...
public class TextRoiMod extends Roi {

	/**
	 *
	 */
	private static final long serialVersionUID = 6998120114741829207L;
	...
	/** Creates a TextRoiMod.*/
	public TextRoiMod(int x, int y, String text) {
		super(x, y, 1, 1);
		init(text, null);
	}
	
	/** Use this constructor as a drop-in replacement for ImageProcessor.drawString(). */
	public TextRoiMod(String text, double x, double y, Font font) {
		super(x, y, 1, 1);
		drawStringMode = true;
	...
	/** Creates a TextRoiMod using sub-pixel coordinates.*/
	public TextRoiMod(double x, double y, String text) {
		super(x, y, 1.0, 1.0);
		init(text, null);
	}

	/** Creates a TextRoiMod using the specified location and Font.
	 * @see ij.gui.Roi#setStrokeColor
	 * @see ij.gui.Roi#setNonScalable
	 * @see ij.ImagePlus#setOverlay(ij.gui.Overlay)
	 */
	public TextRoiMod(int x, int y, String text, Font font) {
		super(x, y, 1, 1);
		init(text, font);
	}

	/** Creates a TextRoiMod using the specified sub-pixel location and Font. */
	public TextRoiMod(double x, double y, String text, Font font) {
		super(x, y, 1.0, 1.0);
		init(text, font);
	}

	/** Creates a TextRoiMod using the specified sub-pixel location, size and Font. */
	public TextRoiMod(double x, double y, double width, double height, String text, Font font) {
	...
	/** @deprecated */
	public TextRoiMod(int x, int y, String text, Font font, Color color) {
		super(x, y, 1, 1);
		if (font==null) font = new Font(name, style, size);
		instanceFont = font;
		IJ.error("TextRoiMod", "API has changed. See updated example at\nhttp://imagej.nih.gov/ij/macros/js/TextOverlay.js");
	}

	public TextRoiMod(int x, int y, ImagePlus imp) {
		super(x, y, imp);
		...
		if (firstTime) {
			...
			theText[0] = line1a;
		if (previousRoi!=null && (previousRoi instanceof TextRoiMod)) {
			firstMouseUp = false;
			previousRoi = null;
		}
		instanceFont = new Font(name, style, size);
	...
	void drawText(Graphics g) {
		...
		if (angle!=0.0) {
			at = g2d.getTransform();
			double cx = nonScalable?xi:screenXD(getRotationCenter().xpoints[0]);
			double cy = nonScalable?yi:screenYD(getRotationCenter().ypoints[0]);
	...
	/** Returns the name of the default font. Use getCurrentFont().getName()
		 to get the name of the font that this TextRoiMod is using. */
	public static String getDefaultFontName() {
		return name;
	}

	/** Returns the default font size. Use getCurrentFont().getSize()
		 to get the size of the font that this TextRoiMod is using. */
	public static int getDefaultFontSize() {
		return size;
	}

	/** Returns the default font style. Use getCurrentFont().getStyle()
		 to get the style of the font that this TextRoiMod is using. */
	...
	public static void setGlobalJustification(int justification) {
		...
		if (imp!=null) {
			Roi roi = imp.getRoi();
			if (roi instanceof TextRoiMod) {
				((TextRoiMod)roi).setJustification(justification);
				imp.draw();
			}
		}
	}
	...
	/** Sets the global font face, size and style that will be used by
		TextRoiMods interactively created using the text tool. */
	...
	/** Sets the font face, size, style and antialiasing mode that will 
		be used by TextRoiMods interactively created using the text tool. */
	public static void setFont(String fontName, int fontSize, int fontStyle, boolean antialiased) {
		...
		if (imp!=null) {
			Roi roi = imp.getRoi();
			if (roi instanceof TextRoiMod) {
				((TextRoiMod)roi).setAntialiased(antialiased);
				((TextRoiMod)roi).setCurrentFont(new Font(name, style, size));
				imp.draw();
			}
		}
	}
	...
	protected void handleMouseUp(int screenX, int screenY) {
		...
		if (width<5 && height<5 && imp!=null && previousRoi==null) {
			...
			TextRoiMod roi = new TextRoiMod(ox, oy, line1a);
	...
	private String getAddSelectionScript(String code) {
		code += "roi = new TextRoiMod("+x+", "+y+", \""+text()+"\", font);\n";
	...
	/** Returns a copy of this TextRoiMod. */
	public synchronized Object clone() {
		TextRoiMod tr = (TextRoiMod)super.clone();
