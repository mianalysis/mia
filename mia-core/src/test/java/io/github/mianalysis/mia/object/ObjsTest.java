package io.github.mianalysis.mia.object;

import org.junit.jupiter.api.Disabled;
import org.junit.jupiter.api.Test;
import org.junit.jupiter.params.ParameterizedTest;
import org.junit.jupiter.params.provider.EnumSource;

import ome.units.UNITS;
import io.github.mianalysis.mia.expectedobjects.VolumeTypes;
import io.github.mianalysis.mia.object.coordinates.ObjI;
import io.github.mianalysis.mia.object.coordinates.volume.CoordinateSetFactoryI;
import io.github.mianalysis.mia.object.coordinates.volume.PointOutOfRangeException;
import io.github.mianalysis.mia.object.coordinates.volume.SpatCal;
import io.github.mianalysis.mia.process.exceptions.IntegerOverflowException;

import static org.junit.jupiter.api.Assertions.*;

public class ObjsTest {
    @ParameterizedTest
    @EnumSource(VolumeTypes.class)
    public void testGetFirstPresent(VolumeTypes volumeType) {
        // Setting calibration parameters
        double dppXY = 0.02;
        double dppZ = 0.1;
        String calibratedUnits = "µm";
        SpatCal calibration = new SpatCal(dppXY, dppZ, calibratedUnits, 1, 1, 1);

        ObjsI collection = ObjsFactories.getDefaultFactory().createFromSpatCal("Obj", calibration, 1, 0.02, UNITS.SECOND);
        CoordinateSetFactoryI factory = VolumeTypes.getFactory(volumeType);

        collection.createAndAddNewObjectWithID(factory, 0);
        collection.createAndAddNewObjectWithID(factory, 1);
        collection.createAndAddNewObjectWithID(factory, 2);

        ObjI firstObj = collection.getFirst();
        assertNotNull(firstObj);
        assertEquals(0, firstObj.getID());

    }

    @Test
    public void testGetFirstAbsent() {
        // Setting calibration parameters
        double dppXY = 0.02;
        double dppZ = 0.1;
        String calibratedUnits = "µm";
        SpatCal calibration = new SpatCal(dppXY, dppZ, calibratedUnits, 1, 1, 1);

        ObjsI collection = ObjsFactories.getDefaultFactory().createFromSpatCal("TestObj", calibration, 1, 0.02, UNITS.SECOND);
        ObjI firstObj = collection.getFirst();
        assertNull(firstObj);

    }

    @ParameterizedTest
    @EnumSource(VolumeTypes.class)
    public void testGetSpatialLimits(VolumeTypes volumeType) throws IntegerOverflowException, PointOutOfRangeException {
        // Setting calibration parameters
        double dppXY = 0.02;
        double dppZ = 0.1;
        String calibratedUnits = "µm";
        SpatCal calibration = new SpatCal(dppXY, dppZ, calibratedUnits, 10, 3, 12);

        // Creating the Objs
        ObjsI collection = ObjsFactories.getDefaultFactory().createFromSpatCal("Obj", calibration, 1, 0.02, UNITS.SECOND);

        // Adding objects
        CoordinateSetFactoryI factory = VolumeTypes.getFactory(volumeType);
        ObjI obj = collection.createAndAddNewObjectWithID(factory, 0);
        obj.addCoord(3, 1, 6);
        obj.addCoord(2, 2, 8);

        obj = collection.createAndAddNewObjectWithID(factory, 1);
        obj.addCoord(3, 2, 2);
        obj.addCoord(2, 2, 9);

        obj = collection.createAndAddNewObjectWithID(factory, 2);
        obj.addCoord(4, 1, 2);
        obj.addCoord(6, 2, 10);

        // Getting expected spatial limits
        int[][] expected = new int[][] { { 0, 9 }, { 0, 2 }, { 0, 11 } };

        // Checking actual values
        int[][] actual = collection.getSpatialLimits();

        for (int i = 0; i < expected.length; i++) {
            assertArrayEquals(expected[i], actual[i]);
        }
    }

    @ParameterizedTest
    @EnumSource(VolumeTypes.class)
    public void testGetTimepointLimits(VolumeTypes volumeType) {
        // Setting calibration parameters
        double dppXY = 0.02;
        double dppZ = 0.1;
        String calibratedUnits = "µm";
        SpatCal calibration = new SpatCal(dppXY, dppZ, calibratedUnits, 1, 1, 1);

        // Creating the Objs
        ObjsI collection = ObjsFactories.getDefaultFactory().createFromSpatCal("Obj", calibration, 1, 0.02, UNITS.SECOND);

        // Adding objects
        CoordinateSetFactoryI factory = VolumeTypes.getFactory(volumeType);
        ObjI obj = collection.createAndAddNewObjectWithID(factory, 0);
        obj.setT(9);

        obj = collection.createAndAddNewObjectWithID(factory, 1);
        obj.setT(3);

        obj = collection.createAndAddNewObjectWithID(factory, 2);
        obj.setT(12);

        int[] expected = new int[] { 3, 12 };
        int[] actual = collection.getTemporalLimits();

        assertArrayEquals(expected, actual);

    }

    @ParameterizedTest
    @EnumSource(VolumeTypes.class)
    public void testGetLargestID(VolumeTypes volumeType) {
        // Setting calibration parameters
        double dppXY = 0.02;
        double dppZ = 0.1;
        String calibratedUnits = "µm";
        SpatCal calibration = new SpatCal(dppXY, dppZ, calibratedUnits, 1, 1, 1);

        // Creating the Objs
        ObjsI collection = ObjsFactories.getDefaultFactory().createFromSpatCal("Obj", calibration, 1, 0.02, UNITS.SECOND);

        // Adding objects
        CoordinateSetFactoryI factory = VolumeTypes.getFactory(volumeType);
        collection.createAndAddNewObjectWithID(factory, 4);
        collection.createAndAddNewObjectWithID(factory, 7);
        collection.createAndAddNewObjectWithID(factory, 2);

        assertEquals(7, collection.getLargestID());

    }

    @Test
    @Disabled
    public void testConvertObjectsToImageSingleColour() {
    }

    @Test
    @Disabled
    public void testConvertObjectsToImageSingleColourNoTemplateImage() {
    }

    @Test
    @Disabled
    public void testConvertObjectsToImageRandomColour() {
    }

    @Test
    @Disabled
    public void testConvertObjectsToImageMeasurementColour() {
    }

    @Test
    @Disabled
    public void testConvertObjectsToImageIDColour() {
    }

    @Test
    @Disabled
    public void testConvertObjectsToImageParentIDColour() {
    }

    @ParameterizedTest
    @EnumSource(VolumeTypes.class)
    public void testGetByEquals(VolumeTypes volumeType) throws IntegerOverflowException, PointOutOfRangeException {
        // Setting calibration parameters
        double dppXY = 0.02;
        double dppZ = 0.1;
        String calibratedUnits = "µm";
        SpatCal calibration = new SpatCal(dppXY, dppZ, calibratedUnits, 10, 4, 12);

        // Creating the Objs
        ObjsI collection = ObjsFactories.getDefaultFactory().createFromSpatCal("Obj", calibration, 1, 0.02, UNITS.SECOND);

        // Adding objects
        CoordinateSetFactoryI factory = VolumeTypes.getFactory(volumeType);
        ObjI obj1 = collection.createAndAddNewObjectWithID(factory, 1);
        obj1.addCoord(3, 2, 2);
        obj1.addCoord(2, 2, 9);

        ObjI obj2 = collection.createAndAddNewObjectWithID(factory, 0);
        obj2.addCoord(3, 2, 2);
        obj2.addCoord(2, 2, 9);
        obj2.addCoord(3, 1, 6);
        obj2.addCoord(2, 2, 8);

        ObjI obj3 = collection.createAndAddNewObjectWithID(factory, 2);
        obj3.addCoord(4, 1, 2);
        obj3.addCoord(6, 2, 10);

        ObjI oj4 = collection.createAndAddNewObjectWithID(factory, 2);
        oj4.addCoord(4, 1, 2);
        oj4.addCoord(6, 2, 10);
        oj4.addCoord(3, 2, 2);
        oj4.addCoord(2, 2, 9);

        // Creating a test object with the same coordinates as one of the other objects
        ObjsI testObjects = ObjsFactories.getDefaultFactory().createFromSpatCal("Test", calibration, 1, 0.02, UNITS.SECOND);
        ObjI testObj = testObjects.createAndAddNewObjectWithID(factory, 5);
        testObj.addCoord(3, 1, 6);
        testObj.addCoord(2, 2, 8);
        testObj.addCoord(3, 2, 2);
        testObj.addCoord(2, 2, 9);

        ObjI actual = collection.getByEqualsIgnoreNameAndID(testObj);

        assertEquals(obj2, actual);
        assertEquals(obj2.getID(), actual.getID());

    }
}