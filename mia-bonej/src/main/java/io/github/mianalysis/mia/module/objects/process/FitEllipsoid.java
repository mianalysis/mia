...
/**
 * Fit ellipsoids to all objects in a collection using
 * "<a href="https://imagej.net/BoneJ">BoneJ</a>". Fit ellipsoids can be stored
 * either as new objects, or replacing the input object coordinates.<br>
 * <br>
 * Note: If updating input objects with ellipsoid coordinates, measurements
...
public class FitEllipsoid extends Module {
    ...
    */
    ...
    /**
     * Objects from workspace to which ellipsoids will be fit. Measurements made by
     * this module are associated with these input objects, irrespective of whether
     * the fit ellipsoids are also stored as objects.
     */
    ...
    */
    ...
    /**
     * Controls which object coordinates are used for ellipsoid fitting:<br>
     * <ul>
     ...
     * are passed to the ellipsoid fitter. Surface coordinates are calculated using
     * 6-way connectivity.</li>
     * </ul>
     */
    public static final String FITTING_MODE = "Fitting mode";

    /**
     * When selected, all axes of the the fit ellipsoids must be shorter than the
     * length specified by "Maximum axis length". This helps filter out mis-fit
     * ellipsoids and prevents unnecessary, massive memory use when storing
     * ellipsoids.
     */
    public static final String LIMIT_AXIS_LENGTH = "Limit axis length";

    /**
     * Maximum length of any fit ellipsoid axis as measured in pixel units. This is
    ...
    */
    ...
    /**
     * Controls whether the fit ellipsoid is stored as an object in the
     * workspace:<br>
     * <ul>
     * <li>"Create new objects" Fit ellipsoids are stored as new objects in the
     * workspace (name specified by "Output objects"). Ellipsoids are "solid"
     * objects, irrespective of whether they were only fit to input object surface
     * coordinates. Ellipsoid objects are children of the input objects to which
     * they were fit. If outputting ellipsoid objects, any measurements are still
     * only applied to the corresponding input objects.</li>
     * <li>"Do not store" (default) The ellipsoid coordinates are not stored.</li>
     * <li>"Update input objects" The coordinates of the input object are removed
     * and replaced with the fit ellipsoid coordinates. Note: Measurements
     * associated with the input object (e.g. spatial measurements) will still be
     * available, but may no longer be valid.</li>
     * </ul>
     */
    public static final String OBJECT_OUTPUT_MODE = "Object output mode";

    /**
     * Name assigned to output ellipsoid objects if "Object output mode" is in
     * "Create new objects" mode.
     */
    ...
    */
    ...
    public void addMeasurements(ObjI inputObject, EllipsoidCalculator calculator) {
        if (calculator == null || calculator.getCentroid() == null) {
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.X_CENT_PX, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.X_CENT_CAL, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.Y_CENT_PX, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.Y_CENT_CAL, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.Z_CENT_SLICE, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.Z_CENT_CAL, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_1_PX, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_1_CAL, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_2_PX, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_2_CAL, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_3_PX, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_3_CAL, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.SURFACE_AREA_PX, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.SURFACE_AREA_CAL, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.VOLUME_PX, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.VOLUME_CAL, Double.NaN));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.ORIENTATION_1, Math.toDegrees(Double.NaN)));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.ORIENTATION_2, Math.toDegrees(Double.NaN)));
            inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.SPHERICITY, Double.NaN));
        ...
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.X_CENT_PX, centres[0]));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.X_CENT_CAL, centres[0] * dppXY));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.Y_CENT_PX, centres[1]));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.Y_CENT_CAL, centres[1] * dppXY));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.Z_CENT_SLICE, centres[2] * dppXY / dppZ));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.Z_CENT_CAL, centres[2] * dppZ));

        double[] radii = calculator.getRadii();
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_1_PX, radii[0]));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_1_CAL, radii[0] * dppXY));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_2_PX, radii[1]));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_2_CAL, radii[1] * dppXY));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_3_PX, radii[2]));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.RADIUS_3_CAL, radii[2] * dppXY));

        double surfaceArea = calculator.getSurfaceArea();
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.SURFACE_AREA_PX, surfaceArea));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.SURFACE_AREA_CAL, surfaceArea * dppXY * dppXY));

        double volume = calculator.getVolume();
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.VOLUME_PX, volume));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.VOLUME_CAL, volume * dppXY * dppXY * dppXY));

        double[] orientations = calculator.getOrientationRads();
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.ORIENTATION_1, Math.toDegrees(orientations[0])));
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.ORIENTATION_2, Math.toDegrees(orientations[1])));

        double sphericity = calculator.getSphericity();
        inputObject.addMeasurement(MeasurementFactories.getDefaultFactory().createMeasurement(Measurements.SPHERICITY, sphericity));
